...
export function RewardAvatarPicker({ level, busy, selectedKey, onSelect }: { level: number; busy: boolean; selectedKey?: string; onSelect: (avatar: RewardAvatar) => void }) {
  return <section className="reward-avatar-collection" aria-label="Earned avatars"><div className="reward-section-heading"><div><p className="reward-eyebrow">MAKE IT YOURS</p><h3>Your avatar collection</h3><p className="reward-intro">Your academy crew. Unlock each collectible through practice, choose your favorite, then save your profile.</p></div></div>
    <div className="reward-avatar-grid">{rewardAvatars.map(avatar => <button type="button" key={avatar.key} className={`reward-avatar-choice${selectedKey === avatar.key ? ' is-selected' : ''}`} aria-label={`${level < avatar.unlockLevel ? 'Locked' : 'Select'} ${avatar.title} avatar${level < avatar.unlockLevel ? `, level ${avatar.unlockLevel}` : ''}`} aria-pressed={selectedKey === avatar.key} disabled={busy || level < avatar.unlockLevel} onClick={() => onSelect(avatar)}><img src={avatar.path} alt="" loading="lazy"/><strong>{avatar.title}</strong><span className="reward-avatar-story">{avatar.description}</span><small>{level < avatar.unlockLevel ? `Level ${avatar.unlockLevel}` : selectedKey === avatar.key ? 'Selected' : 'Unlocked'}</small></button>)}</div>
  </section>
}
