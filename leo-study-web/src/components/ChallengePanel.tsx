...
import type { AcademyChallenge, useAcademyProgression } from '../hooks/useAcademyProgression'
import './ChallengePanel.css'

export function ChallengePanel({ progression, onPractice }: { progression: ReturnType<typeof useAcademyProgression>; onPractice: (challenge: AcademyChallenge) => void }) {
  ...
  return <section className="academy-challenges" aria-label="Practice challenges">
    ...
    <div className="challenge-grid">{status?.challenges.filter(item => item.cadence === cadence).map(item => <article className={`challenge-card${item.claimed ? ' is-complete' : ''}`} key={item.id}>
      ...
      <h3>{item.title}</h3>{item.category ? <span className="challenge-category">{item.category}</span> : null}{item.description ? <p className="challenge-description">{item.description}</p> : null}<div className="challenge-progress-label"><span>{item.progress} / {item.target}</span><span>{Math.round(item.progress / item.target * 100)}%</span></div>
