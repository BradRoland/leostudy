...
export function FrameCollection({ level, selectedKey, avatarUrl, onAvatarError, onSelect, onSave, saving, disabled, error, success }: Props) {
  const [filter, setFilter] = useState<'unlocked' | 'all'>('unlocked')
  const [saveRequested, setSaveRequested] = useState(false)
  const unlocked = profileDecorationCatalog.filter(frame => frame.unlockLevel <= level)
  const next = profileDecorationCatalog.find(frame => frame.unlockLevel > level)
