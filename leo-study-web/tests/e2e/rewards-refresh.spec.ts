...
test('earned avatars and frames save while locked rewards stay unavailable', async ({ page }, info) => {
  ...
  const cardBounds = await frame.boundingBox()
  const nameBounds = await frame.locator('.profile-decoration-copy strong').boundingBox()
  ...
  await expect(page.getByRole('button', { name: 'Unlocked', exact: true })).toHaveAttribute('aria-pressed', 'true')
  ...
  await expect(page.getByRole('region', { name: 'Your frame collection' }).getByRole('status')).toContainText('All changes saved')
...
test('solo setup supports keyboard cancellation and starts both games with chosen options', async ({ page }, info) => {
  await signIn(page)
  for (const game of [
    { path: '/games/matching', opener: 'Start Matching', title: 'Matching Settings', shell: '.match-session-shell-running', exit: 'Exit Match' },
    { path: '/games/speed', opener: 'Start Speed Test', title: 'Speed Test Settings', shell: '.speed-session-shell', exit: 'Exit Test' },
  ]) {
    await page.goto(game.path)
    const opener = page.getByRole('button', { name: game.opener, exact: true })
    await opener.click()
    const dialog = page.getByRole('dialog', { name: game.title, exact: true })
    await expect(dialog).toBeVisible()
    await expect(dialog.getByRole('button', { name: 'All codes', exact: true })).toBeFocused()
    await dialog.getByRole('button', { name: 'Penal Code', exact: true }).click()
    await dialog.getByRole('button', { name: '60 seconds Go a little deeper' }).click()
    await expect(dialog.getByRole('button', { name: 'Penal Code', exact: true })).toHaveAttribute('aria-pressed', 'true')
    await dialog.getByRole('button', { name: 'Start', exact: true }).focus()
    ...
    await opener.click()
    await expect(dialog.getByRole('button', { name: 'Penal Code', exact: true })).toHaveAttribute('aria-pressed', 'true')
    await expect(dialog.getByRole('button', { name: '60 seconds Go a little deeper' })).toHaveAttribute('aria-pressed', 'true')
    ...
    mkdirSync('.artifacts/rewards-refresh.local', { recursive: true })
    await page.screenshot({ path: `.artifacts/rewards-refresh.local/setup-${game.path.split('/').pop()}-${info.project.name}.png` })
...
test('account security keeps password controls accessible and validates before saving', async ({ page }, info) => {
  await signIn(page)
  await page.goto('/profile')
  ...
  await expect(page.getByRole('region', { name: 'Sign-in methods' })).toContainText(email)
  const section = page.getByRole('region', { name: 'Change your password' })
  const inputs = section.locator('input')
  await expect(section.getByRole('button', { name: 'Change Password', exact: true })).toBeDisabled()
  await inputs.nth(0).fill('Test-validation-only!')
  await inputs.nth(1).fill('Deliberate-mismatch!')
  await expect(inputs.nth(0)).toHaveAttribute('type', 'password')
  await section.getByRole('button', { name: 'Toggle new password visibility' }).click()
  await expect(inputs.nth(0)).toHaveAttribute('type', 'text')
  await section.getByRole('button', { name: 'Change Password', exact: true }).click()
  await expect(page.locator('.academy-security-card .bad')).toHaveText('Passwords do not match.')
  await expect(page.getByRole('region', { name: 'Reset study progress' }).getByRole('button', { name: 'Reset Progress and Data' })).toBeVisible()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth + 1)).toBe(true)
  mkdirSync('.artifacts/rewards-refresh.local', { recursive: true })
  await page.screenshot({ path: `.artifacts/rewards-refresh.local/security-${info.project.name}.png`, fullPage: true })
})
