-- Add a 45-second sudden-death phase to 1v1 Code Blaster.
-- Once active, the effective rope KO threshold shrinks so a close match can end quickly.
...
declare
  ...
  v_blaster_effective_rope_limit integer;
  v_blaster_win_condition text;
  v_blaster_sudden_death_active boolean := false;
  v_score_gap integer;
begin
  ...
  v_blaster_win_condition := coalesce(
    nullif(v_room.settings->>'blaster_win_condition', ''),
    case when lower(coalesce(v_room.settings->>'blaster_sudden_death', 'false')) = 'true' then 'death' else 'timed' end
  );
  v_blaster_sudden_death_active := v_room.game_type = 'blaster'
    and now() >= v_room.started_at + make_interval(secs => 45);
  v_blaster_effective_rope_limit := case
    when v_blaster_sudden_death_active then greatest(260, round(v_blaster_rope_limit * 0.45)::integer)
    else v_blaster_rope_limit
  end;
  ...
  if v_room.game_type = 'blaster' then
    ...
    if coalesce(v_score_gap, 0) >= v_blaster_effective_rope_limit then
      update public.room_players
...
begin
  ...
  v_blaster_sudden_death_active := now() >= v_room.started_at + make_interval(secs => 45);
  v_blaster_effective_rope_limit := case
    when v_blaster_sudden_death_active then greatest(260, round(v_blaster_rope_limit * 0.45)::integer)
    else v_blaster_rope_limit
  end;
  ...
  if v_blaster_sudden_death_active and coalesce(v_score_gap, 0) >= v_blaster_effective_rope_limit then
    update public.room_players
    set current_round = greatest(current_round, coalesce(v_room.rounds, current_round) + 1),
        finished_at = coalesce(finished_at, now()),
        last_seen = now()
