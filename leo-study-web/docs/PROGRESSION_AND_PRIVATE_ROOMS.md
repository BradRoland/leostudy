...
Home offers two daily challenges drawn from a catalog of 100 goals. The deterministic rotation covers every goal once in 50 days, then repeats. A migration preserves the current day’s existing goals and begins the new rotation at the next midnight UTC. Goals cover four free practice modes, three code sets, accuracy, improvement and variety, awarding 60–120 XP. See `DAILY_CHALLENGE_CATALOG.md` for the full list. Weekly challenges reward ten completed sessions (250 XP), and practice on three distinct days (200 XP). A session needs at least five answered questions in recorded practice tests or solo games. Opening or abandoning a game does not count. Flashcard browsing and bot warmups do not count toward these particular session challenges.
...
- `backend/staging-blender-avatars-check.mjs`: all seven images, actual WebP upload and profile persistence, earned locks, mobile layout and challenge catalog notice.
